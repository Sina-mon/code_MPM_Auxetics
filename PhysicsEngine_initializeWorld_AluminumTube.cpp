...
// ----------------------------------------------------------------------------
void PhysicsEngine::initializeWorld_AluminumTube(void)
{
	...
	{// initialize GP mediator
		d3_Length_Grid = glm::dvec3(0.1, 0.1, 0.1);
		i3_Cells = glm::ivec3(80, 80, 80);
...
//		double dLength = 0.040;
		...
		double dRadius_Outer = 0.2*0.5*dDiameter;
		double dRadius_Inner = 0.0;//dRadius_Outer - dThickness;
...
//			thisMP->d3_Momentum = thisMP->d3_Mass * thisMP->d3_Velocity;
	...
	{// platen material points ------------------------------------------------ platen MP
		...
		double dRadius_Outer = 0.2*0.7*dDiameter;
		double dRadius_Inner = 0.0*0.3*dDiameter;
		double dOffset = d_Offset;
		//double dLength = 1.0*dOffset;

//		glm::dvec3 d3Center = glm::dvec3(0.5,0.5,0.5) * d3_Length_Grid;
		glm::dvec3 d3Center = glm::dvec3(0.0,0.5,1.0) * d3_Length_Grid;
		d3Center.y = dLength + 2.0*d3_Length_Cell.y;
...
//			thisMP->d3_Momentum = thisMP->d3_Mass * thisMP->d3_Velocity;
	...
	dTimeEnd = 0.0;
	d_TimeIncrement_Maximum = 0.5e-8;
