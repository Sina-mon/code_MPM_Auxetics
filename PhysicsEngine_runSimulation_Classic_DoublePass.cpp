...
// ----------------------------------------------------------------------------
int PhysicsEngine::runSimulation_Classic_DoublePass(double dTimeIncrement_Total)
{
		...
		#pragma omp parallel
		{
			...
			for(unsigned int index_MP = 0; index_MP < allMaterialPoint.size(); index_MP += 1)
			{
				...
				for(int index_AGP = 0; index_AGP < GP_Mediator_Thread.v_adjacentGridPoints.size(); index_AGP++)
				{
					...
					BasesFunction_Thread.calculateBases(thisMP->d3_Position, thisAGP->d3Position, d3_Length_Cell);
			...
			for(unsigned int index_MP = 0; index_MP < allMaterialPoint.size(); index_MP++)
			{
				...
				for(int index_AGP = 0; index_AGP < GP_Mediator_Thread.v_adjacentGridPoints.size(); index_AGP++)
				{
					...
					double dDistance = glm::length(thisMP->d3_Position - thisAGP->d3Position);
					// find kernel value
...
//					glm::dvec3 d3ShapeGradient = BasesFunction_Thread.d3_ShapeGradient;
					...
					if(dDistance_Normalized < 1.0)
					{
						...
						{// atomic grid operations
							...
							glm::dvec3 d3ShapeGradient = glm::normalize(thisMP->d3_Position - thisAGP->d3Position);
							thisAGP->d3_Kernel_ContactGradient += d3ShapeGradient * dShapeValue;
						}
						omp_unset_lock(vGridPoint_Lock[index_GP]);
					}
//					else
//					{
//						thisAGP->d_Kernel_Contact += 0.0;
//						thisAGP->d3_Kernel_ContactGradient += glm::dvec3(0.0, 0.0, 0.0);
//					}
				}
			}
...
*/
				...
				for(unsigned int index_AGP = 0; index_AGP < vMP_AGP[index_MP].size(); index_AGP++)
				{
					...
					{ // avoid atomic operations if there is only 1 thread running
						...
						// momentum
						thisAGP->d3Momentum += dShapeValue * (thisMP->d3_Mass * thisMP->d3_Velocity);

						// internal forces
						double dVolume = thisMP->d_Volume;
						thisAGP->d3Force.x += -dVolume * (d3ShapeGradient.x*thisMP->d6_Stress[0] + d3ShapeGradient.y*thisMP->d6_Stress[3] + d3ShapeGradient.z*thisMP->d6_Stress[5]);
						thisAGP->d3Force.y += -dVolume * (d3ShapeGradient.y*thisMP->d6_Stress[1] + d3ShapeGradient.x*thisMP->d6_Stress[3] + d3ShapeGradient.z*thisMP->d6_Stress[4]);
						thisAGP->d3Force.z += -dVolume * (d3ShapeGradient.z*thisMP->d6_Stress[2] + d3ShapeGradient.x*thisMP->d6_Stress[5] + d3ShapeGradient.y*thisMP->d6_Stress[4]);

						// external forces
						thisAGP->d3Force += dShapeValue*thisMP->d3_Force_External;
					}
					else
					{ // use atomics
						// mass
						...
						{// atomic grid operations
							...
							thisAGP->d3Force.x += -dVolume * (d3ShapeGradient.x*thisMP->d6_Stress[0] + d3ShapeGradient.y*thisMP->d6_Stress[3] + d3ShapeGradient.z*thisMP->d6_Stress[5]) + dShapeValue*thisMP->d3_Force_External.x;
							thisAGP->d3Force.y += -dVolume * (d3ShapeGradient.y*thisMP->d6_Stress[1] + d3ShapeGradient.x*thisMP->d6_Stress[3] + d3ShapeGradient.z*thisMP->d6_Stress[4]) + dShapeValue*thisMP->d3_Force_External.y;
							thisAGP->d3Force.z += -dVolume * (d3ShapeGradient.z*thisMP->d6_Stress[2] + d3ShapeGradient.x*thisMP->d6_Stress[5] + d3ShapeGradient.y*thisMP->d6_Stress[4]) + dShapeValue*thisMP->d3_Force_External.z;
						}
						omp_unset_lock(vGridPoint_Lock[index_GP]);
					}
				}
			...
			for(unsigned int index_MP = 0; index_MP < v_MarkedMaterialPoints_Displacement.size(); index_MP += 1)
			{
				...
				{ // sina, might need mutexes over here
					...
					thisAGP->d3Velocity = thisMP->d3_Velocity;
					thisAGP->d3Momentum = thisAGP->d3_Mass * thisMP->d3_Velocity;
			...
			for(unsigned int index_MP = 0; index_MP < allMaterialPoint.size(); index_MP += 1)
			{
				...
				for(unsigned int index_AGP = 0; index_AGP < vMP_AGP[index_MP].size(); index_AGP++)
				{
					...
					if(glm::length(thisAGP->d3_Mass) > d_Mass_Minimum)
						thisMP->d3_Velocity += dShapeValue * (thisAGP->d3Force/thisAGP->d3_Mass) * dTimeIncrement;
				}
			}
			...
			for(unsigned int index_MP = 0; index_MP < allMaterialPoint.size(); index_MP += 1)
			{
				...
				for(unsigned int index_AGP = 0; index_AGP < vMP_AGP[index_MP].size(); index_AGP++)
				{
					...
					if(glm::length(thisAGP->d3_Mass) > d_Mass_Minimum)
					{
						if(nThreads == 1)
						{ // avoid atomic operations if there is only 1 thread running
							thisAGP->d3Velocity.x += dShapeValue * (thisMP->d3_Mass.x * thisMP->d3_Velocity.x) / thisAGP->d3_Mass.x;
							thisAGP->d3Velocity.y += dShapeValue * (thisMP->d3_Mass.y * thisMP->d3_Velocity.y) / thisAGP->d3_Mass.y;
							thisAGP->d3Velocity.z += dShapeValue * (thisMP->d3_Mass.z * thisMP->d3_Velocity.z) / thisAGP->d3_Mass.z;
						}
						else
						{
//							omp_set_lock(vGridPoint_Lock[index_GP]);
