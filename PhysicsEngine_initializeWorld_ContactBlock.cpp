...
//			thisGridPoint->b3_Fixed = glm::bvec3(true, true, true);
	...
	double dDiameter = 2.0*0.005;
	double dLength = 10.0*d3_Length_Cell.y;//1.0*dDiameter;
	if(true)
...
//			v_MarkedMaterialPoints_Momentum.push_back(thisMP);
	...
	{// tube 2 material points ------------------------------------------------ tube MP
		double dRadius_Outer = 0.8*dDiameter;
		double dRadius_Inner = 0.0;//0.25*dDiameter;
		double dOffset = 1.0/2.0*d3_Length_Cell.x;
		d_Offset = dOffset;
		...
		glm::dvec3 d3Center = glm::dvec3(0.5,0.5,0.5) * d3_Length_Grid;
		d3Center.y = 1.3*dLength + 2.0*d3_Length_Cell.y;

		std::vector<MaterialPoint *> thisMaterialDomain = MP_Factory.createDomain_Tube(d3Center, glm::dvec3(_PI/2.0,0.0,0.0), dRadius_Outer, dRadius_Inner, 6.0*d3_Length_Cell.y, dOffset);
//		std::vector<MaterialPoint *> thisMaterialDomain = MP_Factory.createDomain_QuarterTube(d3Center, glm::dvec3(-_PI/2.0,0.0,0.0), dRadius_Outer, dRadius_Inner, dLength, dOffset);
		for(unsigned int index_MP = 0; index_MP < thisMaterialDomain.size(); index_MP++)
		{// assign material point initial values
			MaterialPoint *thisMP = thisMaterialDomain[index_MP];
...
//			thisMP->i_MaterialType = _VONMISESHARDENING;
			...
			double dMass = 1.0*7800.0 * thisMP->d_Volume;
			thisMP->d3_Mass = glm::dvec3(dMass, dMass, dMass);

			thisMP->d_ElasticModulus = 1.0*200.0e9;
			thisMP->d_Viscosity = 0.0;
			thisMP->d_PoissonRatio = 0.33;
			thisMP->d_YieldStress = 150.0e6;
		...
		{// send to MP vectors
			...
			if(true && thisMP->d3_Position.y > d3Center.y + 0.2*dLength)
			{
				thisMP->b_DisplacementControl = true;
				thisMP->d3_Velocity = glm::dvec3(0.0,-50.0,0.0);
...
//		sDescription += "Grid Point count: " + Script(allGridPoint.size()) + "\n";
		...
		{
			sDescription += "Target yield: " + Script(v_MarkedMaterialPoints_Stress_Monitor[0]->d_YieldStress, 3) + " N/m^2" + "\n";
			sDescription += "Target Modulus: " + Script(v_MarkedMaterialPoints_Stress_Monitor[0]->d_ElasticModulus, 3) + " N/m^2" + "\n";
		}
