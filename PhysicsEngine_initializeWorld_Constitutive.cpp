...
// ----------------------------------------------------------------------------
void PhysicsEngine::initializeWorld_Constitutive(void)
{
	...
	{// initialize GP mediator
		d3_Length_Grid = glm::dvec3(0.020, 0.020, 0.020);
		i3_Cells = 2*glm::ivec3(10, 10, 10);
...
//			thisGridPoint->b3_Fixed = glm::bvec3(true, true, true);
	...
	double dLength = 3.0*dDiameter;
	if(true)
	{// tube material points -------------------------------------------------- tube MP
		double dRadius_Outer = 0.4*0.5*dDiameter;
...
//			thisMP->d3_Momentum = thisMP->d3_Mass * thisMP->d3_Velocity;
		...
		{// send to MP vectors
			...
			if(glm::abs(thisMP->d3_Position.y - 0.0*dLength) < 0.5*dLength)
			{
	...
	{// platen material points ------------------------------------------------ platen MP
		...
		glm::dvec3 d3Center = glm::dvec3(0.5,0.5,0.5) * d3_Length_Grid;
		d3Center.y = dLength + 0.0*d3_Length_Cell.y;

		std::vector<MaterialPoint *> thisMaterialDomain = MP_Factory.createDomain_Tube(d3Center, glm::dvec3(_PI/2.0,0.0,0.0), dRadius_Outer, dRadius_Inner, dOffset, dOffset);
//		std::vector<MaterialPoint *> thisMaterialDomain = MP_Factory.createDomain_QuarterTube_T2(d3Center, glm::dvec3(-_PI/2.0,0.0,0.0), dRadius_Outer, dRadius_Inner, 4.0*d3_Length_Cell.y, dOffset);
//		std::vector<MaterialPoint *> thisMaterialDomain = MP_Factory.createDomain_QuarterTube_T2(d3Center, glm::dvec3(-_PI/2.0,0.0,0.0), dRadius_Outer, dRadius_Inner, dOffset, dOffset);
		for(unsigned int index_MP = 0; index_MP < thisMaterialDomain.size(); index_MP++)
		{// assign material point initial values
			MaterialPoint *thisMP = thisMaterialDomain[index_MP];
...
//			thisMP->d3_Momentum = thisMP->d3_Mass * thisMP->d3_Velocity;
		...
		{// send to MP vectors
			...
			if(true)
			{
				...
				thisMP->d3_Velocity = glm::dvec3(0.0,1.0,0.0);
